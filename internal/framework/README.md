...
The resource's document is generated by `github.com/magodo/terraform-plugin-framework-docs`, which inspect the resource's schema defintion and generate a comprehensive document automatically. There is no manual intervene needed to modify the document. All resources are supposed to implement a method `RenderOption()`, which returns a document render option that can add some more information to the generated document:

- Subcategory: The subcategory of the resource, which shall be the belong AzureDevops *area* of this resource.
- Examples: The examples that will be added to the document. These examples shall be defined in the `<name>_resource_example.go` file, and used both at here and in the acctest, to ensure the examples are verified.
- ImportId: The import by id format and example.
- IdentityExamples: The import by identity examples.

Documents are generated by running `go generate` at the project root directory.
